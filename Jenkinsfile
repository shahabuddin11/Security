pipeline {
   ...
   stages {
       ...
       stage('Audit OpenAPI files') {
        steps {
            audit repositoryName: "${param.git_repo_source}", branchName: "${param.git_repo_branch}", credentialsId: '42crunch-token-id', minScore: 75, platformUrl: 'https://platform.42crunch.com/api/v1/collections', logLevel: 'DEBUG', shareEveryone: 'READ_ONLY'
                }
             }
   }
}
